...
class Parser {
  ...
  handlePrintln(statement) {
    let stmt = new PrintlnStmt(
      statement,
      this.evaluator,
      this.log,
    );
  ...
  handleUsing(statement) {
    let stmt = new UsingStmt(
      statement,
      this.evaluator,
      this.environment
    );
  ...
  handleStatement(statement) {
    ...
    if (statement[0].type == 'USING') {
      return this.handleUsing(statement)
    };

    if (statement[0].type == 'GET') {
      return this.handleGet(statement)
    };
    ...
    if (statement[0].type == 'SHOWLN') {
      return this.handlePrintln(statement);
    };
